...
- **Domain** (`SoloLife.Domain`) — entities (`Entity` base gives a `string Id`, uuidv7 text generated by the DB), enums. No dependencies.
...
- **Infrastructure** write operations return `InfraResult` (carries the affected `string Id`, uuidv7 text).
- **Api** is the *only* layer that returns `IActionResult`. Controllers extend `ApiControllerBase`, which:
  ...
  - exposes `CurrentUserId` (read from the JWT `id` claim, falling back to `NameIdentifier`) — pass this into commands; never trust a user id from the request body,
